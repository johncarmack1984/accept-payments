...
const route = getRouteApi("/i/$token")
...
function demoInvoice(status: InvoiceStatus): PublicInvoice {
  return {
    ...
    line_items: [
      {
        description: "Engineering — June",
        quantity: 40,
        unit_amount_cents: 22_500,
      },
      {
        description: "On-call retainer",
        quantity: 1,
...
export function InvoicePage() {
  ...
  const [state, setState] = useState<State>(
    demo
      ? { status: "ready", invoice: demoInvoice(demo) }
      : { status: "loading" }
  )
  ...
  return (
    <main className="grid min-h-svh place-items-center bg-muted/30 p-4 sm:p-6">
      ...
      {state.status === "ready" ? (
        <InvoiceView invoice={state.invoice} />
      ) : null}
      {state.status === "error" ? (
...
function InvoiceView({ invoice }: { invoice: PublicInvoice }) {
  ...
  // Line items carry no id; the list is a fixed snapshot per invoice, so
  // position-derived keys are stable for its lifetime.
  const items = useMemo(
    () =>
      invoice.line_items.map((item, position) => ({
        ...item,
        key: `line-${position}`,
      })),
    [invoice.line_items]
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.key}>
                ...
                <TableCell className="text-right tabular-nums">
                  {money(
                    item.quantity * item.unit_amount_cents,
                    invoice.currency
                  )}
                </TableCell>
              </TableRow>
