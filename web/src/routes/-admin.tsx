                ...
                invoices.map((invoice) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {money(
                        lineItemsTotal(invoice.line_items),
                        invoice.currency
                      )}
                    </TableCell>
...
type Row = { id: string; description: string; quantity: string; unit: string }
...
function CreateInvoiceForm({
  ...
  const total = rows.reduce(
    (sum, row) =>
      sum +
      (Number(row.quantity) || 0) * Math.round((Number(row.unit) || 0) * 100),
    0
  )

  function updateRow(id: string, patch: Partial<Row>) {
    setRows((rs) =>
      rs.map((row) => (row.id === id ? { ...row, ...patch } : row))
    )
  }
